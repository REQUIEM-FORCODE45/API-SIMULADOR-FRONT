...
const AddDynamicForm = ({ name, internalJson, onBack }) => {
    const [jsonFields, setJsonFields] = useState([]);
    const [formData, setFormData] = useState({});
    const { theme } = useContext(ThemeContext);
    const { appendToText, concatenatedText, NewGlmlist , setList } = useSidebar();
    ...
    // Genera el texto plano
    const generatePlainText = () => {
        const nonEmptyFields = Object.entries(formData)
            .filter(([key, value]) => value !== null && value !== "" && !readOnlyFields.includes(key))
            .map(([key, value]) => `    ${key}   ${value};`) // Formatea cada línea
        
        const plainText = `object ${name} {\n${nonEmptyFields.join("\n")}\n}`;
        appendToText(plainText);
    };

    const addComponentList = async () => {
        const result = await Swal.fire({
            title: `¿Deseas agregar el componente ${name}?`,
            text: "Confirma que quieres agregarlo a la lista.",
            icon: 'question',
        ...
        if (result.isConfirmed) {
            ...
            if (!(name in List)) {
                List[name] = { key: name, items: [] };
            }
    
            List[name].items.push(formData);
            setList(List);
            console.log(List);
            ...
            await Swal.fire(
                '¡Agregado!',
                `El componente ${name} fue agregado exitosamente.`,
                'success'
            );
        }
    };
    ...
    return (
        <PageContainer>
            <FormContainer>
                                        ...
                                        style={{
                                            backgroundColor:
                                                theme === "light"
                                                    ...
                                                    : readOnlyFields.includes(field)
                                                    ? "#9e9e9e"
                                                    : "#cfcfcf", // Fondo gris para solo lectura
                                        }}
                                    />
                                )}
                            </div>
                ...
                <button onClick={addComponentList} className="btn btn-primary">
                    Add component
                </button>
                <Divider />
            </FormContainer>
